...
def widget_showoff(
    ...
    plt.plot(range(1, 50), [random.random() for i in range(1, 50)], "r-")
    plt.plot(range(1, 50), [random.random() for i in range(1, 50)], "k--")
...
@funix(
    ...
    widgets={"show": "switch", ("input1", "input2"): slider(0, 100)},
    input_layout=[
        [{"markdown": "**Guess a number**"}],
        [{"argument": "input1", "width": 6}, {"argument": "input2", "width": 6}],
        [{"dividing": "Cheat Option", "position": "left"}],
        [{"argument": "cheat", "width": 12}],
    ],
    show_source=True,
)
def guess(input1: int = 0, input2: int = 0, cheat: bool = False) -> str:
    global randomNumber
    if cheat:
...
@funix(
    ...
    widgets={"a": "sheet", "b": ["sheet", slider(0, 1, 0.01)]},
    show_source=True,
)
...
@funix(
    input_layout=[
        ...
        [{"argument": "x", "width": 4}, {"argument": "y", "width": 4}],  # row 2
    ],
    output_layout=[
...
@funix(
    ...
    output_layout=[
        [{"index": 0}],  # row 1
        [{"markdown": "**Download Link**", "width": 2}, {"index": 1}],  # row 2
        [{"markdown": "**Visit the repo**"}, {"index": 2}],  # row 3
    ],
    show_source=True,
)
def layout_example(
    user_name: str = "texteainc", repo_name: str = "json-viewer"
) -> (Image, File, Markdown):
